...
| `RUST-I12` | **Medium, open:** Successful GUI startup still logs one Qwen embedding reclassification destination collision; all 83 models remain visible. Exact path evidence is in the private GUI log identified by the ledger. | Model-library classification/naming owner | Retain both entries and their files; no merge, deletion, or relocation selected. This warning does not block the startup correction. | Investigate destination naming against repository and selected-artifact identity before proposing a no-loss correction; prove both entries remain independently usable. | Admission of the classification/naming fix, a missing model, or any proposed automatic collision resolution |
