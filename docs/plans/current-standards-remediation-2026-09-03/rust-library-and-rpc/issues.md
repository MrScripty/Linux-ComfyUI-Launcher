...
| --- | --- | --- | --- | --- | --- |
| `RUST-I8` | **Critical:** The prior migration caller moved before lifecycle admission, ignored refusal, and guessed rollback. The approved legacy replacement now owns movement, marker, persistence, and retained authority; source integration remains unaccepted. | Rust lifecycle/store and model-migration owners | Legacy implementation and independent narrow review complete; isolated strict Clippy fails on disconnected recovery integration and lint findings. No warning suppression or held-consumer scope expansion is admitted. | Retain exact-hash relocation/migration evidence in the execution ledger; inventory and admit the production core recovery boundary, then pass affected dual-mode tests and strict Clippy before a source commit. Admitted relocation and broader C3 remain open. | Materially different caller contract or integration boundary |
