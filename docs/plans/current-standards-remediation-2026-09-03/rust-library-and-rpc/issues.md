...
| --- | --- | --- | --- | --- | --- |
| `RUST-I9` | **High, C3:** A started queued successor accepts pause but cannot settle while a paused destination head retains its claim. `PreparedDownloadTask::run_owned` awaits `wait_for_turn` before checking pause; the existing `restored_legacy_root_alias_blocks_a_canonical_successor` fixture establishes the reachable public-start state. | Rust download lifecycle owner | Separate next C3 slice from stalled network waits: wake queued pause into owned settlement without destination effects; retain exact admission and FIFO position. Source review confirms the gap; no dynamic acceptance yet. | Public queued pause reaches durable Paused while the head remains paused; resume retains ordering; cancellation and persistence failure preserve exact ownership. | Implementing queued-pause settlement or changing the public queued-pause contract |
| `RUST-I8` | **Critical, resolved for the legacy checkpoint:** Migration previously moved before admission, ignored refusal, and guessed rollback. The owned replacement coordinates movement, marker, persistence, and retained authority; cancellation preserves snapshot custody across revocation. | Rust lifecycle/store and model-migration owners | Incremental core boundary accepted after independent review, dual full-package tests, strict lint, and unchanged-consumer compilation. No warnings were suppressed. | Preserve exact-hash evidence in the ledger. Pending cleanup/unknown state remains fail-closed; admitted relocation and broader C3 remain open. | Materially different caller contract, authority, or integration boundary |
