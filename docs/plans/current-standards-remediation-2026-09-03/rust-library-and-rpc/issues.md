...
| --- | --- | --- | --- | --- | --- |
| `RUST-I8` | **Critical:** C3's held-identity queue draft exposes retained-capability/path disagreement in legacy relocation. `api/migration.rs` renames the directory and updates its marker before HF relocation, ignores `Ok(false)`, and attempts rollback on `Err`. Store relocation has no expected-old comparison and erases publication uncertainty. | Rust lifecycle/store and model-migration owners; program scope decision | Blocked pending permission to include the migration caller and owned relocation transition. No raw-path fallback, silent legacy disablement, or weakened assertion is admitted. | Prove preflight before movement, cancellation/outcome agreement, expected-old persistence, exact capability/queue transfer, and uncertain-publication custody. Existing relocation regression must pass at the final source hash. | User scope decision or a materially different caller contract |
