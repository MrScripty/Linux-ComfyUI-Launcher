...
/// Once the move syscall may have taken effect, both locations remain owned
/// until the relocation owner reconciles the outcome. No rollback is implied.
#[derive(Debug)]
pub(crate) enum DestinationMoveError {
    NotMoved(io::Error),
...
impl DownloadRecoveryDestination {
    /// Validate a move without creating destination directories. The caller
    /// serializes Pumas operations at both locations through the entire move.
    pub(crate) fn preflight_move_to_absent(&self, target: &Self) -> io::Result<()> {
        if !cfg!(target_os = "linux") {
            return Err(io::Error::new(
                io::ErrorKind::Unsupported,
                "No-replace directory relocation requires Linux",
            ));
        }
        if self.authority.root_identity != target.authority.root_identity
            || target.model_relative.starts_with(&self.model_relative)
        {
            return Err(invalid_capability_path());
        }
    ...
    /// Read the optional marker through retained directory authority. A present
    /// marker must be a regular file containing a JSON object.
    pub(crate) fn read_marker(&self) -> io::Result<Option<Value>> {
        ...
        let file = match directory.open_with(".pumas_download", &options) {
            Ok(file) => file,
        ...
        let value: Value = serde_json::from_reader(file.into_std())
            .map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error))?;
        if !value.is_object() {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "Download marker must be a JSON object",
            ));
        }
        self.directory(false)?;
        Ok(Some(value))
    ...
    /// Move one directory without replacement and retain the same held inode.
    /// Parent creation may remain after `NotMoved`. Any failure after rename
    /// is indeterminate and must not trigger cleanup at either location.
    ///
    /// Pumas must exclude its own competing source/target mutations. Linux
    /// cannot compare the source inode atomically with rename: external changes
    /// are checked before and after, without claiming hostile-substitution
    /// prevention during that interval.
    pub(crate) fn move_to_absent(
        &self,
        target: &Self,
    ) -> std::result::Result<Self, DestinationMoveError> {
        self.move_to_absent_inner(target, || {})
    }

    fn move_to_absent_inner(
        &self,
        target: &Self,
        after_rename: impl FnOnce(),
    ) -> std::result::Result<Self, DestinationMoveError> {
        self.preflight_move_to_absent(target)
            .map_err(DestinationMoveError::NotMoved)?;
        #[cfg(not(target_os = "linux"))]
        {
            let _ = after_rename;
            Err(DestinationMoveError::NotMoved(io::Error::new(
                io::ErrorKind::Unsupported,
                "No-replace directory relocation requires Linux",
            )))
        }
        #[cfg(target_os = "linux")]
        {
            let prepared = (|| -> io::Result<_> {
                ...
                let source_parent_relative = self
                    .model_relative
                    .parent()
                ...
                let source_name = self
                    ...
                    .ok_or_else(invalid_capability_path)?;
                let source_parent =
                    open_directory_chain(&self.authority.root, source_parent_relative, false)?;
                let target_parent_relative = target
                    .model_relative
                    .parent()
                    .ok_or_else(invalid_capability_path)?;
                let target_name = target
                    ...
                    .file_name()
                    .ok_or_else(invalid_capability_path)?;
                let target_tail_parent = target
                    .creation_anchor
                    .tail
                    .parent()
                    .ok_or_else(invalid_capability_path)?;
                let target_parent = open_directory_chain(
                    &target.creation_anchor.directory,
                    target_tail_parent,
                    true,
                )?;
                let target_parent_identity = directory_identity(&target_parent)?;
                if directory_identity(&open_directory_chain(
                    &target.authority.root,
                    target_parent_relative,
                    false,
                )?)? != target_parent_identity
                    || directory_identity(&open_directory_chain(
                        &source_parent,
                ...
                Ok((
                    ...
                    target_parent_relative.to_path_buf(),
                    target_parent_identity,
                    source_identity,
                ))
            })()
            ...
            rename_directory_noreplace(&source_parent, &source_name, &target_parent, &target_name)
                .map_err(|error| {
                    // These rejected syscall preconditions leave the source name
                    // untouched. Unclassified I/O outcomes are deliberately not
                    // interpreted as permission to roll back or retry elsewhere.
                    if matches!(
                        error.raw_os_error(),
                        Some(
                            libc::EEXIST
                        ...
                        DestinationMoveError::NotMoved(error)
                    } else {
                        DestinationMoveError::Indeterminate(error)
                    }
                })?;
            after_rename();
            (|| -> io::Result<Self> {
                source_parent.open(".")?.sync_all()?;
                target_parent.open(".")?.sync_all()?;
                self.authority.require_current()?;
                target.authority.require_current()?;
                if directory_identity(&open_directory_chain(
                    &self.authority.root,
                ...
                let moved = Self {
                    ...
                    model_relative: target.model_relative.clone(),
                    display_path: target.display_path.clone(),
                    held: self.held.clone(),
                    creation_anchor: Arc::new(CreationAnchor {
                        directory: target_parent,
                        relative: target_parent_relative,
                        identity: target_parent_identity,
                        tail: PathBuf::from(target_name),
                    }),
                    file_parents: self.file_parents.clone(),
                };
                moved.directory(false)?;
                Ok(moved)
            })()
            .map_err(DestinationMoveError::Indeterminate)
        }
    }
...
/// The owned directory descriptors and C strings remain alive through the
/// syscall. Names contain exactly one validated component. This helper owns
/// the only unsafe mechanism; cap-std's safe rename permits replacement.
#[cfg(target_os = "linux")]
#[allow(unsafe_code)]
fn rename_directory_noreplace(
    ...
    let source_name =
        std::ffi::CString::new(source_name.as_bytes()).map_err(|_| invalid_capability_path())?;
    let target_name =
        std::ffi::CString::new(target_name.as_bytes()).map_err(|_| invalid_capability_path())?;
    // SAFETY: both Dir owners keep their descriptors valid for this call;
    // both CString owners provide live NUL-terminated names. renameat2 borrows
    // these inputs, retains no pointers, and does not transfer descriptor ownership.
    let result = unsafe {
        libc::renameat2(
            source.as_raw_fd(),
            source_name.as_ptr(),
...
mod tests {
    #[cfg(target_os = "linux")]
    #[test]
    fn destination_move_refuses_fifo_marker_without_waiting_for_a_writer() {
        ...
        let status = std::process::Command::new("mkfifo")
            .arg(temp.path().join("source/.pumas_download"))
            .status()
            .unwrap();
        assert!(status.success());
        let target = root.resolve(std::path::Path::new("target")).unwrap();
        assert!(source.preflight_move_to_absent(&target).is_err());
        assert!(!temp.path().join("target").exists());
    }

    #[cfg(target_os = "linux")]
    #[test]
    fn no_replace_syscall_preserves_both_existing_directories() {
        let temp = tempfile::TempDir::new().unwrap();
        ...
        let error = super::rename_directory_noreplace(
            &parent,
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn destination_move_reports_post_move_authority_loss_as_indeterminate() {
        let temp = tempfile::TempDir::new().unwrap();
        let root = super::DownloadDestinationRoot::open(temp.path()).unwrap();
        let source = root.resolve(std::path::Path::new("source")).unwrap();
        source.prepare().unwrap();
        std::fs::write(temp.path().join("source/sentinel"), b"keep").unwrap();
        let target = root.resolve(std::path::Path::new("target")).unwrap();
        let outcome = source.move_to_absent_inner(&target, || {
            std::fs::rename(temp.path().join("target"), temp.path().join("moved-target")).unwrap();
        });
        assert!(matches!(
            outcome,
            Err(super::DestinationMoveError::Indeterminate(_))
        ));
        assert!(
            !temp.path().join("source").exists(),
            "must not attempt rollback"
        );
        assert_eq!(
            std::fs::read(temp.path().join("moved-target/sentinel")).unwrap(),
            b"keep"
        );
    }
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn destination_move_preserves_held_files_and_marker_without_replacement() {
        use std::io::Write;
        let temp = tempfile::TempDir::new().unwrap();
        let root = super::DownloadDestinationRoot::open(temp.path()).unwrap();
        let source = root.resolve(std::path::Path::new("old/model")).unwrap();
        source.prepare().unwrap();
        source
            .open_part("nested/weights.gguf", false)
            .unwrap()
            .write_all(b"partial")
            .unwrap();
        source
            .write_marker(&serde_json::json!({"repo_id":"acme/model"}))
            .unwrap();
        let target = root.resolve(std::path::Path::new("new/model")).unwrap();
        source.preflight_move_to_absent(&target).unwrap();
        assert!(
            !temp.path().join("new").exists(),
            "preflight must be read-only"
        );
        let inode = super::directory_identity(&source.directory(false).unwrap()).unwrap();
        let moved = source.move_to_absent(&target).unwrap();
        assert_eq!(
            super::directory_identity(&moved.directory(false).unwrap()).unwrap(),
            inode
        );
        assert_eq!(moved.identity(), target.identity());
        assert_eq!(moved.part_len("nested/weights.gguf").unwrap(), Some(7));
        assert_eq!(
            std::fs::read(temp.path().join("new/model/nested/weights.gguf.part")).unwrap(),
            b"partial"
        );
        assert!(temp.path().join("new/model/.pumas_download").is_file());
        assert!(!temp.path().join("old/model").exists());
        assert!(
            source.remove_marker().is_err(),
            "old authority must not follow relocation"
        );
    }

    #[cfg(target_os = "linux")]
    #[test]
    fn destination_move_refuses_an_existing_target_and_lost_source() {
        let temp = tempfile::TempDir::new().unwrap();
        let root = super::DownloadDestinationRoot::open(temp.path()).unwrap();
        let source = root.resolve(std::path::Path::new("source")).unwrap();
        source.prepare().unwrap();
        ...
        source.preflight_move_to_absent(&target).unwrap();
        std::fs::create_dir(temp.path().join("target")).unwrap();
        std::fs::write(temp.path().join("target/sentinel"), b"keep").unwrap();
        assert!(matches!(
            source.move_to_absent(&target),
            Err(super::DestinationMoveError::NotMoved(_))
        ));
        assert_eq!(
            std::fs::read(temp.path().join("target/sentinel")).unwrap(),
            b"keep"
        );
        let absent = root.resolve(std::path::Path::new("absent")).unwrap();
        std::fs::rename(temp.path().join("source"), temp.path().join("old-source")).unwrap();
        assert!(matches!(
            source.move_to_absent(&absent),
            Err(super::DestinationMoveError::NotMoved(_))
        ));
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn destination_move_refuses_lost_root_and_invalid_marker() {
        let temp = tempfile::TempDir::new().unwrap();
        ...
        source.prepare().unwrap();
        let target = root.resolve(std::path::Path::new("target")).unwrap();
        std::fs::write(path.join("source/.pumas_download"), b"[]").unwrap();
        assert!(source.preflight_move_to_absent(&target).is_err());
        std::fs::remove_file(path.join("source/.pumas_download")).unwrap();
        std::fs::rename(&path, temp.path().join("old-library")).unwrap();
        assert!(matches!(
            source.move_to_absent(&target),
            Err(super::DestinationMoveError::NotMoved(_))
        ));
        assert!(!temp.path().join("old-library/target").exists());
    }
